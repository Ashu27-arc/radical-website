...
export default function CounselorForm() {
  ...
  const handleSubmit = async () => {
    ...
    if (!consent) return showError("Please accept the consent to proceed");
    ...
    <div className="relative overflow-hidden md:overflow-visible">
      ...
      <div className="space-y-6 relative z-10">
        <div className="relative w-full">
          ...
          <span
            className="absolute font-normal left-4 top-4 text-sm text-[#78787899] pointer-events-none 
        ...
        <Button
          ...
          disabled={!consent}
          loading={loading}
          className={`w-full h-14 !rounded-[14px] text-lg font-bold !border-none shadow-lg hover:opacity-90 transition-opacity 
                                    ${
                                      !consent
                                        ? "!bg-[#ccc] !text-white cursor-not-allowed"
                                        : "!bg-gradient-to-r !from-[#63CDB4] !to-[#099CF6]"
                                    }
                                `}
        />
